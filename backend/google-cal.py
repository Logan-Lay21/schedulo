...
def create_event(
    ...
    event = {
        ...
        "attendees": [{"email": email} for email in attendees],  # List of email addresses
        "reminders": reminders,
        "colorId": colorId,
    ...
    try:
        ...
        for event in events:
            ...
            if private.get('classID') == course and private.get('assignmentName') == assignment and private.get('aiGenerated'):
                ...
                event_found = True
        if not event_found:
          print(f"No event found with classID {course} and assignmentName {assignment}.")
...
def main():
  ...
  try:
    ...
    eight_days_later = (datetime.datetime.utcnow() + datetime.timedelta(days=8)).isoformat() + "Z"
    ...
    for event in events:
      ...
      assignment_name = private_properties.get('assignmentName', "No assignment name")
